...
namespace Saffron\Blog\Model\ResourceModel\Post;
...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    {
        parent::_construct();
        $this->_init('Saffron\Blog\Model\Post', 'Saffron\Blog\Model\ResourceModel\Post');
     ...
     * @param array|int|\Saffron\Blog\Model\Category  $category
     * @return $this
     */
    public function addCategoryFilter($category)
    {
        if (!$this->getFlag('category_filter_added')) {
            if ($category instanceof \Saffron\Blog\Model\Category) {
    ...
    protected function _afterLoad()
    {
        ...
        if (count($items)) {
            ...
            $select = $connection->select()->from(['cps' => $this->getTable('saffron_blog_post_store')])
            ...
            $select = $connection->select()->from(['cps' => $this->getTable('saffron_blog_post_category')])
                ->where('cps.post_id IN (?)', $items);
    ...
    protected function _renderFiltersBefore()
    {
        foreach(array('store', 'category') as $key) {
            if ($this->getFilter($key)) {
                $this->getSelect()->join(
                    [$key.'_table' => $this->getTable('saffron_blog_post_'.$key)],
