...
class AdminNotificationFeed extends \Magento\AdminNotification\Model\Feed
{
    ...
    public function getFeedUrl()
    {
        ...
        $modulesParams = array();
        foreach($this->getSaffronModules() as $key => $module) {
            $key = str_replace('Saffron_', '', $key);
    ...
    /**
     * Get Saffron Modules Info
     *
     * @return $this
     */
    protected function getSaffronModules()
    {
        $modules = array();
        foreach($this->_moduleList->getAll() as $moduleName => $module) {
            if ( strpos($moduleName, 'Saffron_') !== false && $this->_moduleManager->isEnabled($moduleName) ) {
    ...
    {
        return $this->_cacheManager->load('saffron_admin_notifications_lastcheck');
    }
    ...
    {
        $this->_cacheManager->save(time(), 'saffron_admin_notifications_lastcheck');
        return $this;
    }
