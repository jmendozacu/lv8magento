...
class FormManagement
{
    ...
    public function prepareForm($tabId, $storeId)
    {
        ...
        switch ($tabId) {
            case ManageCheckoutTabsInterface::CUSTOMER_INFO_TAB:
                ...
                $form = $this->createOrderFieldsButton($form);
            ...
            case ManageCheckoutTabsInterface::PAYMENT_METHOD_TAB:
                ...
                $fields = $this->getOrderAttributeFields(
                    $storeId, [self::PAYMENT_STEP, self::PAYMENT_PLACE_ORDER]
                );
                break;
    ...
    public function addGroup(Form $form, $groupId, $title, $enabled)
    {
        ...
        $group->setId($groupId);
        $group->setRenderer(Group::getGroupRenderer());
    ...
    private function prepareAdditionalFields($attributes, $storeId, $moduleDepend)
    {
        ...
        foreach ($attributes as $item) {
            /** @var FieldFactory $fieldModel */
            $fieldModel = $this->fieldFactory->create();

            $storelabel = $item->getFrontendLabel();

            if ($storeId != Field::DEFAULT_STORE_ID) {
                $this->prepareStorelabel($item, $storeId, $storelabel);
            }
            ...
            $fieldModel->setData('label', $storelabel);
     ...
     * @return Form
     */
    private function createOrderFieldsButton($form)
    {
        $form->addField(
            'order-fields-button',
            'button',
            [
                'onclick' => sprintf("window.open('%s');", $this->urlManagement->getUrl('amorderattr/attribute/create')),
    ...
    private function createCustomerFieldsButton($form)
    {
        $form->addField(
            ...
            'button',
            [
                'onclick' => sprintf("window.open('%s');", $this->urlManagement->getUrl('amcustomerattr/attribute/new')),
    ...
    /**
     * @param \Amasty\Orderattr\Model\Attribute\Attribute|\Magento\Eav\Model\Entity\Attribute $attribute
     * @param int $storeId
     * @param string $storeLabel
     */
    private function prepareStorelabel($attribute, $storeId, &$storeLabel)
    {
